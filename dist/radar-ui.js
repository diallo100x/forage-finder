(function(){
  ...
  const intel=window.FFIntel,region=document.querySelector('#regionInput'),status=document.querySelector('#radarStatus');
  let scoped=signals;
  ...
  window.ForageRadar={signals,providers:intel.providerCatalog,searchRegion(q){if(region)region.value=q;applyRegion();return scoped},addSignals(items){signals.push(...items.map(intel.normalize));applyRegion()},explainSpecies(id){return intel.cluster(scoped,id).signals.map(intel.explain)}};
  summarize();
})();
